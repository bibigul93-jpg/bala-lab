...
Если спрашивают о проблемах с растениями — давай конкретные советы.
Поощряй любопытство!
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Я умею:\n🌱 Помогать выращивать микрозелень\n"
        "🤖 Отвечать на ЛЮБЫЕ вопросы о растениях\n"
...
async def choose_plant(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"Факт: {plant['fact']}\n\n"
        f"Задавай мне любые вопросы о растениях — я отвечу! 🔬"
    )
...
async def water(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if already:
        await update.message.reply_text(f"Уже поливал сегодня! День {day} из {plant.get('harvest_days',7)} 🌅", reply_markup=main_keyboard())
    else:
        db.add_points(user_id, 5)
        await update.message.reply_text(
            f"Полив записан! День {day} 💧\n2-3 ст.ложки воды — не переливай!\n\n+5 очков! ⭐",
            reply_markup=main_keyboard()
        )
...
async def measure_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Измерь самый высокий росток 📏\n\nНапиши высоту в мм (например: 15)")
...
async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"Фото дня {day} сохранено! 📸\nФотографируй каждый день — тайм-лапс! 🎬\n\n+15 очков! ⭐", reply_markup=main_keyboard())
...
async def ai_reply(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    waterings = db.get_watering_count(user_id)
    points = db.get_points(user_id)

    ctx = f"Имя: {user_name}. Растение: {plant}. День: {day}. Поливов: {waterings}. Очков: {points}."
